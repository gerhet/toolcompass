/* QRCode.js – Minimal complete QR Code generator (Public Domain / MIT-style) */
(function(global) {
  'use strict';
  ...
  // Utility
  var QRUtil = {
    PATTERN_POSITION_TABLE: [[],[6,18],[6,22],[6,26],[6,30],[6,34],[6,22,38],[6,24,42],[6,26,46],[6,28,50],[6,30,54],[6,32,58],[6,34,62],[6,26,46,66],[6,26,48,70],[6,26,50,74],[6,30,54,78],[6,30,56,82],[6,30,58,86],[6,34,62,90],[6,28,50,72,94],[6,26,50,74,98],[6,30,54,78,102],[6,28,54,80,106],[6,32,58,84,110],[6,30,58,86,114],[6,34,62,90,118],[6,26,50,74,98,122],[6,30,54,78,102,126],[6,26,52,78,104,130],[6,30,56,82,108,134],[6,34,60,86,112,138],[6,30,58,86,114,142],[6,34,62,90,118,146],[6,30,54,78,102,126,150],[6,24,50,76,102,128,154],[6,28,54,80,106,132,158],[6,32,58,84,110,136,162],[6,26,54,82,110,138,166],[6,30,58,86,114,142,170],[6,34,62,90,118,146,170]],
  ...
  function QR8bitByte(data) {
    ...
    if (this.parsedData.length != this.data.length) { this.parsedData.unshift(0xEF); this.parsedData.unshift(0xBB); this.parsedData.unshift(0xBF); }
  }
  ...
  var api = {
    generate: function(text, opts) {
      opts = opts || {};
      var eccLevel = opts.errorCorrectLevel !== undefined ? opts.errorCorrectLevel : 0; // 0=M,1=L,2=H,3=Q
      var eccMap = [0, 1, 3, 2]; // M, L, Q, H
      var typeNumber = opts.typeNumber || -1;
      if (typeNumber < 1) {
        for (var i = 1; i <= 40; i++) {
          try {
            var rsBlocks = QRRSBlock.getRSBlocks(i, eccLevel);
            var totalDataCount = 0;
            for (var j = 0; j < rsBlocks.length; j++) totalDataCount += rsBlocks[j].dataCount;
            var buffer = new QRBitBuffer();
            var data = new QR8bitByte(text);
            buffer.put(data.mode, 4);
            buffer.put(data.getLength(), QRUtil.getLengthInBits(data.mode, i));
            data.write(buffer);
      ...
      var qrcode = new QRCode(typeNumber, eccLevel);
      qrcode.addData(text);
      qrcode.make();
      return qrcode;
    }
